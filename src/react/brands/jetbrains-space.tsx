import type { BrandIcon, BrandIconProps } from '../types'
export const IconBrandJetBrainsSpace: BrandIcon = ({ className, ...props }: BrandIconProps) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    aria-label='JetBrains Space'
    className={`ui-icon-brand ${className ?? ''}`}
    role='img'
    preserveAspectRatio='xMidYMid'
    viewBox='0 0 256 256'
    fill='none'
    {...props}>
    <defs>
      <linearGradient id='jetbrains_space__a' x1='45.082%' x2='55.522%' y1='-1.343%' y2='97.61%'>
        <stop offset='0%' stopColor='#fcf84a' />
        <stop offset='32%' stopColor='#abe682' />
        <stop offset='79%' stopColor='#36cdd2' />
        <stop offset='100%' stopColor='#07c3f2' />
      </linearGradient>
      <linearGradient id='jetbrains_space__b' x1='-2.942%' x2='100.445%' y1='38.992%' y2='62.105%'>
        <stop offset='0%' stopColor='#3bea62' />
        <stop offset='100%' stopColor='#087cfa' />
      </linearGradient>
      <linearGradient id='jetbrains_space__c' x1='-3.308%' x2='111.745%' y1='71.807%' y2='15.462%'>
        <stop offset='0%' stopColor='#009ae5' />
        <stop offset='18%' stopColor='#0490dd' />
        <stop offset='49%' stopColor='#1073c6' />
        <stop offset='89%' stopColor='#2346a1' />
        <stop offset='100%' stopColor='#293896' />
      </linearGradient>
    </defs>
    <path
      fill='url(#jetbrains_space__a)'
      d='M46.36 255.991c91.075-7.978 170.982-63.92 209.64-146.768C208.12 40.799 129.873.033 46.36 0Q35.2 0 23.936.99c-38.612 82.903-30.064 180.109 22.424 255Z'
    />
    <path
      fill='url(#jetbrains_space__b)'
      d='M24.209 1.024a301.4 301.4 0 0 1 134.685 108.199H256C208.263 40.76 130.062-.03 46.6 0Q35.438 0 24.209 1.024'
    />
    <path
      fill='url(#jetbrains_space__c)'
      d='M158.894 109.223C147.187 186.6 46.258 255.99 46.258 255.99c91.781-8.67 172.23-63.622 209.742-146.768z'
    />
  </svg>
)
