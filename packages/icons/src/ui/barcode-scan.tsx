import type { Icon } from './types'

export const IconBarcodeScan: Icon = ({
  size = 24,
  strokeWidth = 1.5,
  className,
  'aria-label': ariaLabel,
  ...props
}) => {
  const isLabelled = Boolean(ariaLabel)

  return (
    <svg
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      width={size}
      height={size}
      strokeWidth={strokeWidth}
      strokeLinecap='round'
      strokeLinejoin='round'
      data-slot='icon-ui-barcode-scan'
      role={isLabelled ? 'img' : undefined}
      aria-hidden={isLabelled ? undefined : true}
      aria-label={ariaLabel}
      focusable={isLabelled ? undefined : false}
      className={`icon-ui ${className ?? ''}`.trim()}
      {...props}>
      <rect
        opacity='.2'
        width='18.57'
        height='18.57'
        rx='3'
        transform='scale(1 -1)rotate(90 21.29 0)'
        fill='currentColor'
      />
      <path d='M6.77 16.64V7.36' stroke='currentColor' />
      <path d='M10.24 16.64V7.36' stroke='currentColor' />
      <path d='M13.72 16.64V7.36' stroke='currentColor' />
      <path d='M17.2 16.64V7.36' stroke='currentColor' />
      <path
        d='M15.93 21.32c2.27 0 3.4 0 4.2-.57a3 3 0 0 0 .64-.65c.58-.8.58-1.93.58-4.2'
        stroke='currentColor'
      />
      <path
        d='M15.97 2.71c2.23 0 3.35 0 4.13.56a3 3 0 0 1 .69.69c.56.78.56 1.9.56 4.14'
        stroke='currentColor'
      />
      <path
        d='M8.07 21.32c-2.27 0-3.4 0-4.2-.58a3 3 0 0 1-.64-.64c-.58-.8-.58-1.93-.58-4.2'
        stroke='currentColor'
      />
      <path
        d='M8.07 2.71c-2.27 0-3.4 0-4.2.58a3 3 0 0 0-.64.65c-.58.79-.58 1.92-.58 4.2'
        stroke='currentColor'
      />
    </svg>
  )
}
