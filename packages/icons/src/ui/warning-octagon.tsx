import type { Icon } from './types'

export const IconWarningOctagon: Icon = ({
  size = 24,
  strokeWidth = 1.5,
  className,
  'aria-label': ariaLabel,
  ...props
}) => {
  const isLabelled = Boolean(ariaLabel)

  return (
    <svg
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      width={size}
      height={size}
      strokeWidth={strokeWidth}
      strokeLinecap='round'
      strokeLinejoin='round'
      data-slot='icon-ui-warning-octagon'
      role={isLabelled ? 'img' : undefined}
      aria-hidden={isLabelled ? undefined : true}
      aria-label={ariaLabel}
      focusable={isLabelled ? undefined : false}
      className={`icon-ui ${className ?? ''}`.trim()}
      {...props}>
      <path
        opacity='.2'
        d='M10.2 2.44c.88-.44 1.32-.66 1.8-.66s.92.22 1.8.66l2.96 1.5 2.74 1.72c.87.54 1.3.81 1.55 1.24s.27.93.32 1.95l.16 3.15-.16 3.15c-.05 1.02-.07 1.53-.32 1.95-.25.43-.68.7-1.55 1.24l-2.74 1.72-2.96 1.5c-.88.44-1.32.66-1.8.66s-.92-.22-1.8-.66l-2.96-1.5-2.74-1.72c-.87-.54-1.3-.81-1.55-1.24s-.27-.93-.32-1.95L2.47 12l.16-3.15c.05-1.02.07-1.53.32-1.95.25-.43.68-.7 1.55-1.24l2.74-1.72z'
        fill='currentColor'
      />
      <path
        d='M9.05 2.86c1.44-.81 2.16-1.22 2.95-1.22s1.5.4 2.95 1.22l1.77 1 1.73 1.02c1.44.84 2.16 1.26 2.55 1.94.4.69.4 1.52.42 3.18l.02 2-.02 2c-.01 1.66-.02 2.5-.42 3.18s-1.11 1.1-2.55 1.94l-1.73 1.02-1.77 1c-1.44.81-2.16 1.22-2.95 1.22s-1.5-.4-2.95-1.22l-1.77-1-1.73-1.02C4.1 18.28 3.39 17.86 3 17.18c-.4-.69-.4-1.52-.42-3.18l-.02-2 .02-2c.01-1.66.02-2.5.42-3.18s1.11-1.1 2.55-1.94l1.73-1.02z'
        stroke='currentColor'
      />
      <path d='M12 6.91v5.64' stroke='currentColor' />
      <path
        d='M12.59 16.5a.59.59 0 1 1-1.18 0 .59.59 0 0 1 1.18 0'
        fill='currentColor'
        stroke='currentColor'
      />
    </svg>
  )
}
