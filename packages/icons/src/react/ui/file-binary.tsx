import type { Icon } from './types'

export const IconFileBinary: Icon = ({
  size = 24,
  strokeWidth = 1.5,
  className,
  'aria-label': ariaLabel,
  ...props
}) => {
  const isLabelled = Boolean(ariaLabel)

  return (
    <svg
      viewBox='0 0 24 24'
      fill='none'
      xmlns='http://www.w3.org/2000/svg'
      width={size}
      height={size}
      strokeWidth={strokeWidth}
      strokeLinecap='round'
      strokeLinejoin='round'
      data-slot='icon-ui-file-binary'
      role={isLabelled ? 'img' : undefined}
      aria-hidden={isLabelled ? undefined : true}
      aria-label={ariaLabel}
      focusable={isLabelled ? undefined : false}
      className={`icon-ui ${className ?? ''}`.trim()}
      {...props}>
      <path
        opacity='.2'
        d='M3.8 13.08V5.72c0-1.87 0-2.8.58-3.4.58-.58 1.52-.59 3.38-.6L10 1.69c1.3-.01 1.96-.02 2.44.27a2 2 0 0 1 .68.68c.3.48.3 1.13.3 2.44 0 1.3 0 1.94.29 2.42q.25.41.67.67c.48.3 1.13.3 2.42.3 1.3 0 1.95 0 2.43.28q.41.26.67.68c.29.47.29 1.12.29 2.42v1.23z'
        fill='currentColor'
      />
      <path
        d='M3.8 13.02V7.76c0-2.83 0-4.25.88-5.12s2.3-.88 5.13-.88h1.9c1.23 0 1.85 0 2.4.22.55.23.98.66 1.85 1.53l1.24 1.24 1.18 1.15c.9.87 1.34 1.3 1.57 1.86.24.56.24 1.19.24 2.43v2.83'
        stroke='currentColor'
      />
      <path d='M13.22 2.27v2.46c0 1.89 0 2.83.58 3.42s1.53.58 3.42.58h2.46' stroke='currentColor' />
      <rect x='3.71' y='15.78' width='3.82' height='6.62' rx='1.91' stroke='currentColor' />
      <rect x='16.51' y='15.78' width='3.82' height='6.62' rx='1.91' stroke='currentColor' />
      <path
        d='m10.2 17.88 1.2-1.1c.71-.65 1.07-.98 1.37-.85.3.14.3.62.3 1.6v4.87'
        stroke='currentColor'
      />
    </svg>
  )
}
